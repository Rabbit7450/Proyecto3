...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!orderId) {
        notify('Error', 'No se proporcionó un ID de pedido válido', 'error');
        setTimeout(() => {
...
async function loadOrderDetails(orderId) {
    try {
        ...
        if (typeof notify === 'function') {
            notify('Error', 'No se pudo cargar la información del pedido: ' + error.message, 'error');
        } else {
...
async function uploadReceipt() {
    ...
    try {
        ...
        if (response.ok && result.success) {
            ...
            notify('Éxito', 'El comprobante ha sido subido correctamente', 'success');
            
            // Opcional: limpiar el input después de un tiempo
            setTimeout(() => {
                removeFile();
            }, 3000);
        } else {
